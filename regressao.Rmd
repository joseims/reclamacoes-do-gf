...
A CAPES é um órgão do MEC que tem a atribuição de acompanhar a pós-graduação na universidade brasileira. 
Uma das formas que ela encontrou de fazer isso e pela qual ela é bastante criticada é através de uma avaliação quantitativa a cada x anos (era 3, mudou para 4). 
...
Uma das maneiras de avaliar a produção dos docentes que a CAPES utiliza é quantificando a produção de artigos pelos docentes.
Os artigos são categorizados em extratos ordenados (A1 é o mais alto), e separados entre artigos em conferências e periódicos.
Usaremos para esse lab a produção em periódicos avaliados com A1, A2 e B1.
...
Se considerarmos que temos apenas uma amostra de todos os programas de pós em CC no Brasil, o que podemos inferir a partir desse modelo sobre a relação entre número de docentes permanentes e produção de artigos em programas de pós? 
    ...
    Aparentemente o modelo explica bem o gráfico, nos casos com poucos docentes podemos ver que existe uma correlação positiva entre o número de docentes e o número de públicações, pelos dados que temos a correlação parece continuar conforme o número de docentes cresce.
    Entrentanto por termos um baixo número de universidades que tem mais de 40 docentes não podemos afirmar isso com muita certeza.
